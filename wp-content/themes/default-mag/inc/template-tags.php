...
if ( ! function_exists( 'default_mag_posted_by' ) ) :
	...
	function default_mag_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'por %s', 'post author', 'default-mag' ),
	...
	function default_mag_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links twp-links"><span class="twp-icon"><i class="fa fa-calendar"></i></span>' . esc_html__( 'Postado em %1$s', 'default-mag' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links twp-links"><span class="twp-icon"><i class="fa fa-tags"></i></span>' . esc_html__( 'Marcado %1$s', 'default-mag' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Deixe um comentário<span class="screen-reader-text"> em %s</span>', 'default-mag' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Editar <span class="screen-reader-text">%s</span>', 'default-mag' ),
					array(
						'span' => array(
							'class' => array(),
						),
        	    ...
        	    <i class="fa fa-clock-o"></i>
        	        ...
        	        if ($site_date_layout_option == 'normal-format') {
        	           the_time(get_option('date_format'));
        	            ...
        	            echo human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ' . __('atrás', 'default-mag');
        	        }
            ...
            if ($post_categories) {
                ...
                foreach ($post_categories as $post_category) {
                    $output .= '<li class="float-left">
                             <a  href="' . esc_url(get_category_link($post_category)) . '" alt="' . esc_attr(sprintf(__('Ver todas as postagens em %s', 'default-mag'), $post_category->name)) . '"> 
